...
import com.atlassian.jira.rest.client.api.{JiraRestClient, UserRestClient}
...
class JiraUserMappingTest(_system: ActorSystem)
  ...
  def this() = {
    this(ActorSystem("JiraUserMappingTest", ConfigFactory.parseString(
      """plugins {
        |  jira {
        |    mapping {
        |      panda = "german"   # Panda's user id in Slack is "panda", while it's "german" in Jira
        |    }
        |  }
        |}
      """.stripMargin)))
  ...
  val jiraUserObject = new JiraUser(null, "german", null, null, null, avatarUris, null)
  when(userClient.getUser("german")).thenReturn(Promises.promise(jiraUserObject))
  ...
  private val slackUserObject = new SlackUser("U124", "panda", None, None, None, None, None, None, None, None, None, None, None, None, None, None)
  ...
  "JiraUserMapping" should {
    "correctly map jira users to slack users" in {
      sut.jiraToSlack(jiraUserObject).get.name should be("panda")
    }

    "correctly map slack users to jira users" in {
      sut.slackToJira(slackUserObject).get.getName should be("german")
    }
  }
}
